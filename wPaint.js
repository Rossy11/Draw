...
(function($)
{
	$.fn.wPaint = function(option, settings)
	{
		...
		{			
            ...
            if(_settings.imageBg) $elem.append(canvas.generateBg('2200', '1300', _settings.imageBg));
	...
	$.fn.wPaint.defaultSettings = {
		...
		fontSizeMax			 : '30',				// max font size in px
		...
		menu 				 : ['undo', 'clear','pencil','eraser','lineWidth','strokeColor'], // menu items - appear in order they are set
		menuOrientation		 : 'vertical',		    // orinetation of menu (horizontal, vertical)
		menuOffsetX			 : 0,					// offset for menu (left)
		menuOffsetY			 : 0,					// offset for menu (top)
	...
	function Canvas(settings, elem)
	{
		...
		this.undoArray = [];
		this.undoCurrent = -1;
		this.undoMax = 21; //todo 撤回的次数,限制20次
	...
	{	
		...
		{
            _self.settings.lineWidth=60 //todo...橡皮大小
			_self.ctx.save();
		...
		getImage: function()
		{
			...
			//$(this.canvasSave).css({display:'none', position: 'absolute', left: 0, top: 0}).attr('width', $(this.canvas).attr('width')).attr('height', $(this.canvas).attr('height')); todo...
			$(this.canvasSave).css({display:'none', position: 'absolute', left: 0, top: 0}).attr('width', 2000).attr('height', 1300);
	...
	MainMenu.prototype = 
	{
		generate: function(canvas, textMenu)
		{
			...
			$.each($canvas.settings.menu, function(i, item)
			{
				switch(item)
				{
					...
					// case 'strokeColor': menuContent.append($('<div class="_wPaint_strokeColorPicker _wPaint_colorPicker" title="' + $canvas.settings.menuTitles.strokeColor + '"></div>')); break; TODO
					case 'strokeColor': menuContent.append($('<div class="_wPaint_strokeColorPicker _wPaint_colorPicker"></div>')); break;
				}
			});
		...
		set_mode: function(_self, $canvas, mode)
		{
			...
			else
			{
                ...
                $canvas.settings.lineWidth=$("select").val();

                //todo
				if(mode=='Pencil'||mode=='Eraser'){
                    missPen=false;
                    $("#missPen").removeClass("activeZ");
				}
